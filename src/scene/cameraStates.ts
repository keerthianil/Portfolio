 ...
 *
 * The two wall shots sit at the edge of the yaw clamp, so panning to the limit
 * and clicking the poster land you in roughly the same place.
 */
export const SHOTS: Record<CameraState, Shot> = {
  ...
  timeline: { position: [-0.34, 1.02, 0.1], target: [-0.44, 0.86, -0.56], fov: 34 },
  taborder: { position: [-1.0, 1.36, 0.5], target: [-2.05, 1.42, -0.5], fov: 40 },
  beforeafter: { position: [1.0, 1.36, 0.5], target: [2.05, 1.42, -0.5], fov: 40 },
};
 ...
 * framing cropped the laptop and the e-reader straight off the sides. The wide
 ...
 * Framing a 1.42m monitor edge to edge in portrait would need either a fisheye
...
export const SHOTS_PORTRAIT: Record<CameraState, Shot> = {
  ...
  timeline: { position: [-0.36, 1.06, 0.2], target: [-0.44, 0.86, -0.56], fov: 46 },
  taborder: { position: [-0.9, 1.36, 0.62], target: [-2.05, 1.42, -0.5], fov: 54 },
  beforeafter: { position: [0.9, 1.36, 0.62], target: [2.05, 1.42, -0.5], fov: 54 },
};
